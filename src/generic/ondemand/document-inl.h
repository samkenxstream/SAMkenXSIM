...
{
  logger::log_headers();
  parser->current_string_buf_loc = parser->dom_parser.doc->string_buf.get();
}
...
simdjson_really_inline SIMDJSON_IMPLEMENTATION::ondemand::array simdjson_result<SIMDJSON_IMPLEMENTATION::ondemand::document>::end() & noexcept { return as_value().end(); }

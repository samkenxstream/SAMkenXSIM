...
```c++
auto [doc, error] = document::parse("[ 1, 2, 3 ]"_pad);
...
```c++
cout << document::parse("[ 1, 2, 3 ]"_pad);
```

If you're wondering why the examples above use `_pad`, it's because the simdjson library requires SIMDJSON_PADDING extra bytes at the end of a string (it doesn't matter if the bytes are initialized). `_pad`
is a way of creating a `padded_string` class, which assures us we have enough allocation.

```c++
padded_string json = "[ 1, 2, 3 ]"_pad;

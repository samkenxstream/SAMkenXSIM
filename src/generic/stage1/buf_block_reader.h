...
public:
  really_inline buf_block_reader(const uint8_t *_buf, size_t len) : buf{_buf}, lenminusstep{len < STEP_SIZE ? 0 : len - STEP_SIZE}, idx{0} {}
  ...
  really_inline bool has_remainder(size_t len) const {
    return idx < len;
  }
  really_inline void get_remainder(uint8_t *tmp_buf, size_t len) const {
